...
from sesamebip158 import PyBIP158

import sesame.server.ws_connection as ws
from sesame.consensus.block_creation import create_unfinished_block
from sesame.consensus.block_record import BlockRecord
from sesame.consensus.pot_iterations import calculate_ip_iters, calculate_iterations_quality, calculate_sp_iters
from sesame.full_node.bundle_tools import best_solution_generator_from_template, simple_solution_generator
from sesame.full_node.full_node import FullNode
from sesame.full_node.mempool_check_conditions import get_puzzle_and_solution_for_coin
from sesame.full_node.signage_point import SignagePoint
from sesame.protocols import farmer_protocol, full_node_protocol, introducer_protocol, timelord_protocol, wallet_protocol
from sesame.protocols.full_node_protocol import RejectBlock, RejectBlocks
from sesame.protocols.protocol_message_types import ProtocolMessageTypes
from sesame.protocols.wallet_protocol import PuzzleSolutionResponse, RejectHeaderBlocks, RejectHeaderRequest
from sesame.server.outbound_message import Message, make_msg
from sesame.types.blockchain_format.coin import Coin, hash_coin_list
from sesame.types.blockchain_format.pool_target import PoolTarget
from sesame.types.blockchain_format.program import Program
from sesame.types.blockchain_format.sized_bytes import bytes32
from sesame.types.coin_record import CoinRecord
from sesame.types.end_of_slot_bundle import EndOfSubSlotBundle
from sesame.types.full_block import FullBlock
from sesame.types.generator_types import BlockGenerator
from sesame.types.mempool_inclusion_status import MempoolInclusionStatus
from sesame.types.mempool_item import MempoolItem
from sesame.types.peer_info import PeerInfo
from sesame.types.unfinished_block import UnfinishedBlock
from sesame.util.api_decorators import api_request, peer_required, bytes_required, execute_task
from sesame.util.generator_tools import get_block_header
from sesame.util.hash import std_hash
from sesame.util.ints import uint8, uint32, uint64, uint128
from sesame.util.merkle_set import MerkleSet
...
class FullNodeAPI:
    ...
    @peer_required
    @api_request
    async def request_peers(self, _request: full_node_protocol.RequestPeers, peer: ws.WSSesameConnection):
    ...
    async def respond_peers(
        self, request: full_node_protocol.RespondPeers, peer: ws.WSSesameConnection
    ) -> Optional[Message]:
    ...
    async def respond_peers_introducer(
        self, request: introducer_protocol.RespondPeersIntroducer, peer: ws.WSSesameConnection
    ) -> Optional[Message]:
    ...
    @peer_required
    @api_request
    async def new_peak(self, request: full_node_protocol.NewPeak, peer: ws.WSSesameConnection) -> Optional[Message]:
        """
    ...
    async def new_transaction(
        self, transaction: full_node_protocol.NewTransaction, peer: ws.WSSesameConnection
    ) -> Optional[Message]:
        """
    ...
    async def new_signage_point_or_end_of_sub_slot(
        self, new_sp: full_node_protocol.NewSignagePointOrEndOfSubSlot, peer: ws.WSSesameConnection
    ) -> Optional[Message]:
        # Ignore if syncing
        if self.full_node.sync_store.get_sync_mode():
            return None
    ...
    async def respond_signage_point(
        self, request: full_node_protocol.RespondSignagePoint, peer: ws.WSSesameConnection
    ) -> Optional[Message]:
        if self.full_node.sync_store.get_sync_mode():
            return None
    ...
    async def declare_proof_of_space(
        self, request: farmer_protocol.DeclareProofOfSpace, peer: ws.WSSesameConnection
    ) -> Optional[Message]:
        """
    ...
    async def signed_values(
        self, farmer_request: farmer_protocol.SignedValues, peer: ws.WSSesameConnection
    ) -> Optional[Message]:
        """
    ...
    async def new_infusion_point_vdf(
        self, request: timelord_protocol.NewInfusionPointVDF, peer: ws.WSSesameConnection
    ) -> Optional[Message]:
        if self.full_node.sync_store.get_sync_mode():
            return None
    ...
    async def new_signage_point_vdf(
        self, request: timelord_protocol.NewSignagePointVDF, peer: ws.WSSesameConnection
    ) -> None:
        if self.full_node.sync_store.get_sync_mode():
            return None
    ...
    async def new_end_of_sub_slot_vdf(
        self, request: timelord_protocol.NewEndOfSubSlotVDF, peer: ws.WSSesameConnection
    ) -> Optional[Message]:
        if self.full_node.sync_store.get_sync_mode():
            return None
        if (
