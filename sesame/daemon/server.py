...
from sesame.cmds.init_funcs import sesame_init
from sesame.daemon.windows_signal import kill
from sesame.server.server import ssl_context_for_root, ssl_context_for_server
from sesame.ssl.create_ssl import get_mozzila_ca_crt
from sesame.util.sesame_logging import initialize_logging
from sesame.util.config import load_config
from sesame.util.json_util import dict_to_json_str
from sesame.util.path import mkdir
from sesame.util.service_groups import validate_service
from sesame.util.setproctitle import setproctitle
from sesame.util.ws_message import WsRpcMessage, create_payload, format_response
...
try:
    ...
    print("Error: Make sure to run . ./activate from the project folder before starting Sesame.")
    quit()
...
service_plotter = "sesame plots create"
...
if getattr(sys, "frozen", False):
    name_map = {
        "sesame": "sesame",
        "sesame_wallet": "start_wallet",
        "sesame_full_node": "start_full_node",
        "sesame_harvester": "start_harvester",
        "sesame_farmer": "start_farmer",
        "sesame_introducer": "start_introducer",
        "sesame_timelord": "start_timelord",
        "sesame_timelord_launcher": "timelord_launcher",
        "sesame_full_node_simulator": "start_simulator",
    }
...
class WebSocketServer:
    ...
    async def exit(self) -> Dict[str, Any]:
        ...
        log.info("sesame daemon exiting in 5 seconds")
...
def launch_plotter(root_path: Path, service_name: str, service_array: List[str], id: str):
    # we need to pass on the possibly altered SESAME_ROOT
    os.environ["SESAME_ROOT"] = str(root_path)
...
def launch_service(root_path: Path, service_command) -> Tuple[subprocess.Popen, Path]:
    ...
    log.debug(f"Launching service with SESAME_ROOT: {os.environ['SESAME_ROOT']}")
...
async def async_run_daemon(root_path: Path) -> int:
    sesame_init(root_path)
    config = load_config(root_path, "config.yaml")
    setproctitle("sesame_daemon")
