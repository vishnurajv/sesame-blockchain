...
from sesame.types.coin_solution import CoinSolution
...
from sesame.util.ints import uint32, uint64
from sesame.wallet.cc_wallet import cc_utils
from sesame.wallet.cc_wallet.cc_utils import CC_MOD, SpendableCC, spend_bundle_for_spendable_ccs, uncurry_cc
from sesame.wallet.cc_wallet.cc_wallet import CCWallet
from sesame.wallet.puzzles.genesis_by_coin_id_with_0 import genesis_coin_id_for_genesis_coin_checker
from sesame.wallet.trade_record import TradeRecord
from sesame.wallet.trading.trade_status import TradeStatus
from sesame.wallet.trading.trade_store import TradeStore
from sesame.wallet.transaction_record import TransactionRecord
from sesame.wallet.util.trade_utils import (
...
# from sesame.wallet.cc_wallet.debug_spend_bundle import debug_spend_bundle
            ...
            for id in offer.keys():
                ...
                if isinstance(wallet, CCWallet):
                    ...
                    if spend_bundle is None:
                        ...
                        to_exclude = spend_bundle.removals()
                    new_spend_bundle = await wallet.create_spend_bundle_relative_sesame(amount, to_exclude)
                else:
    ...
    async def respond_to_offer(self, file_path: Path) -> Tuple[bool, Optional[TradeRecord], Optional[str]]:
        ...
        for coinsol in offer_spend_bundle.coin_solutions:
            ...
            if r:
                ...
                # standard sesame coin
                unspent = await self.wallet_state_manager.get_spendable_coins_for_wallet(1)
                if coinsol.coin in [record.coin for record in unspent]:
                    return False, None, "can't respond to own offer"
                if sesame_discrepancy is None:
                    sesame_discrepancy = get_output_discrepancy_for_puzzle_and_solution(coinsol.coin, puzzle, solution)
                else:
                    sesame_discrepancy += get_output_discrepancy_for_puzzle_and_solution(coinsol.coin, puzzle, solution)
                coinsols.append(coinsol)

        sesame_spend_bundle: Optional[SpendBundle] = None
        if sesame_discrepancy is not None:
            sesame_spend_bundle = await self.wallet_state_manager.main_wallet.create_spend_bundle_relative_sesame(
                sesame_discrepancy, []
            )
            if sesame_spend_bundle is not None:
                for coinsol in coinsols:
                    sesame_spend_bundle.coin_solutions.append(coinsol)
        ...
        for colour in cc_discrepancies.keys():
            if cc_discrepancies[colour] < 0:
                ...
                if sesame_spend_bundle is None:
                    to_exclude: List = []
                else:
                    to_exclude = sesame_spend_bundle.removals()
                ...
                if my_cc_spends is None or my_cc_spends == set():
                    ...
                    if zero_spend_bundle is None:
                        return (
                            False,
                            None,
                            "Unable to generate zero value coin. Confirm that you have sesame available",
                        )
        ...
        now = uint64(int(time.time()))
        if sesame_spend_bundle is not None:
            spend_bundle = SpendBundle.aggregate([spend_bundle, sesame_spend_bundle])
            # debug_spend_bundle(spend_bundle)
            if sesame_discrepancy < 0:
                tx_record = TransactionRecord(
                    confirmed_at_height=uint32(0),
                    created_at_time=now,
                    to_puzzle_hash=token_bytes(),
                    amount=uint64(abs(sesame_discrepancy)),
                    fee_amount=uint64(0),
                    confirmed=False,
                    sent=uint32(10),
                    spend_bundle=sesame_spend_bundle,
                    additions=sesame_spend_bundle.additions(),
                    removals=sesame_spend_bundle.removals(),
                    wallet_id=uint32(1),
                    sent_to=[],
                    trade_id=std_hash(spend_bundle.name() + bytes(now)),
                    type=uint32(TransactionType.OUTGOING_TRADE.value),
                    name=sesame_spend_bundle.name(),
                )
            else:
                tx_record = TransactionRecord(
                    confirmed_at_height=uint32(0),
