...
from sesame.protocols import wallet_protocol
from sesame.protocols.full_node_protocol import RequestProofOfWeight, RespondProofOfWeight
from sesame.protocols.protocol_message_types import ProtocolMessageTypes
from sesame.protocols.wallet_protocol import (
...
from sesame.util.byte_types import hexstr_to_bytes
from sesame.util.errors import Err, ValidationError
from sesame.util.ints import uint32, uint128
from sesame.util.keychain import Keychain
from sesame.util.lru_cache import LRUCache
from sesame.util.merkle_set import MerkleSet, confirm_included_already_hashed, confirm_not_included_already_hashed
from sesame.util.path import mkdir, path_from_root
from sesame.wallet.block_record import HeaderBlockRecord
from sesame.wallet.derivation_record import DerivationRecord
from sesame.wallet.settings.settings_objects import BackupInitialized
from sesame.wallet.transaction_record import TransactionRecord
from sesame.wallet.util.backup_utils import open_backup_file
from sesame.wallet.util.wallet_types import WalletType
from sesame.wallet.wallet_action import WalletAction
from sesame.wallet.wallet_blockchain import ReceiveBlockResult
from sesame.wallet.wallet_state_manager import WalletStateManager
...
class WalletNode:
    ...
    def get_key_for_fingerprint(self, fingerprint: Optional[int]):
        ...
        if len(private_keys) == 0:
            self.log.warning("No keys present. Create keys with the UI, or with the 'sesame keys' program.")
            return None
    ...
    async def on_connect(self, peer: WSSesameConnection):
        if self.wallet_state_manager is None or self.backup_initialized is False:
            return None
    ...
    async def complete_blocks(self, header_blocks: List[HeaderBlock], peer: WSSesameConnection):
    ...
    async def new_peak_wallet(self, peak: wallet_protocol.NewPeakWallet, peer: WSSesameConnection):
        if self.wallet_state_manager is None:
            return None
    ...
    async def _sync(self) -> None:
        ...
        peers: List[WSSesameConnection] = self.server.get_full_node_connections()
        if len(peers) == 0:
    ...
    async def get_additions(self, peer: WSSesameConnection, block_i, additions) -> Optional[List[Coin]]:
        if len(additions) > 0:
    ...
    async def get_removals(self, peer: WSSesameConnection, block_i, additions, removals) -> Optional[List[Coin]]:
        assert self.wallet_state_manager is not None
