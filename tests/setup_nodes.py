...
from sesame.daemon.server import WebSocketServer, create_server_for_daemon, daemon_launch_lock_path, singleton
from sesame.full_node.full_node_api import FullNodeAPI
from sesame.server.start_farmer import service_kwargs_for_farmer
from sesame.server.start_full_node import service_kwargs_for_full_node
from sesame.server.start_harvester import service_kwargs_for_harvester
from sesame.server.start_introducer import service_kwargs_for_introducer
from sesame.server.start_service import Service
from sesame.server.start_timelord import service_kwargs_for_timelord
from sesame.server.start_wallet import service_kwargs_for_wallet
from sesame.simulator.start_simulator import service_kwargs_for_full_node_simulator
from sesame.timelord.timelord_launcher import kill_processes, spawn_process
from sesame.types.peer_info import PeerInfo
from sesame.util.bech32m import encode_puzzle_hash
from sesame.util.block_tools import BlockTools, test_constants
from sesame.util.hash import std_hash
from sesame.util.ints import uint16, uint32
...
async def setup_farmer(
    ...
    config["xsm_target_address"] = encode_puzzle_hash(b_tools.farmer_ph, "xsm")
    config["pool_public_keys"] = [bytes(pk).hex() for pk in b_tools.pool_pubkeys]
    config["port"] = port
    config_pool["xsm_target_address"] = encode_puzzle_hash(b_tools.pool_ph, "xsm")
